  ...
  if (msg.type === 'job') {
    ...
    if (currentJob.seed_hash && currentJob.seed_hash !== currentSeedHash) {
      ...
      try {
        ...
        rxCache = randomx_init_cache(currentSeedHash);
...
function checkDifficulty(hash: Uint8Array, targetHex: string): boolean {
  ...
  const hashRev = Buffer.from(hash).reverse(); // Stratum usa little-endian para comparação às vezes, depende da implementação

  // Comparação simples de bytes (big-endian)
  for (let i = 0; i < 32; i++) {
